\chapter{Server}
\label{ch:server}
A server is a computer program that supplies clients with services. The term 'server' often refers to the machine on which the program is running on.

\section{History of Servers}

\todo{what were the first approaches to servers? how are they used? client-server model?}

\section{Tasks}
The tasks of the server are to manage incoming connections and their requests. It should always be prepared to send sensor data to a client if it is requested.
