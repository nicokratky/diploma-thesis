...
\begin{lstlisting}[caption={Push},captionpos=b,label={code:mlr_push}]
...
\begin{minipage}{\textwidth}
\begin{lstlisting}[caption={Predict},captionpos=b,label={code:mlr_predict}]
...
As the mathematical side of regression analysis requires a lot of calculations to be made using matrices. As C++ does not have an equivalent to matrices, a third party library had to be chosen to compensate this. After comparing several libraries that offer matrix calculations, it was decided to use Eigen.
